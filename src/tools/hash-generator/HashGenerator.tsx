...
// MD5 (SubtleCrypto가 지원 안 해서 직접 구현)
function md5(s: string): string {
  ...
  function ad(x: number, y: number): number { const lsw = (x & 0xffff) + (y & 0xffff); const msw = (x >> 16) + (y >> 16) + (lsw >> 16); return (msw << 16) | (lsw & 0xffff); }
  function rl(n: number, c: number): number { return (n << c) | (n >>> (32 - c)); }
  function cmn(q: number, a: number, b: number, x: number, s: number, t: number): number { return ad(rl(ad(ad(a, q), ad(x, t)), s), b); }
  ...
  function blocks(s: string): number[] { const nblk = ((s.length + 8) >> 6) + 1; const blks = new Array(nblk * 16); let i; for (i = 0; i < nblk * 16; i++) blks[i] = 0; for (i = 0; i < s.length; i++) blks[i >> 2] |= s.charCodeAt(i) << ((i % 4) * 8); blks[i >> 2] |= 0x80 << ((i % 4) * 8); blks[nblk * 16 - 2] = s.length * 8; return blks; }
  const x = blocks(unescape(encodeURIComponent(s)));
...
export default function HashGenerator() {
  ...
  const [results, setResults] = useState<Record<Algo, string>>({} as any);
  const [copiedAlgo, setCopiedAlgo] = useState<Algo | null>(null);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const r: Record<Algo, string> = {} as any;
      for (const algo of ['SHA-1', 'SHA-256', 'SHA-384', 'SHA-512'] as const) {
        try {
          r[algo] = await digest(algo, text);
        } catch {
          r[algo] = '';
        }
      }
  ...
  const copy = async (algo: Algo, value: string) => {
    ...
    <div className="grid gap-6">
      <div>
        ...
        <textarea
          ...
          spellCheck={false}
          className="h-32 w-full rounded-lg border bg-slate-50 p-3 font-mono text-sm focus:border-emerald-400 focus:outline-none"
        />
